...
@fig-proposed-dashboard provides an illustration of our current mock-up of the proposed dashboard. The dashboard will include filters, an interactive map, and summary charts/graphs/tables that us and the partner determine to be insightful. Navigation will be based on tabs for minimal scrolling and intuitive searching. This *Shiny App* will be initially programmed in Python and hosted on Posit Connect Cloud.
...
The project motivation is to improve upon and enhance the current static Boreal Avian Modelling Centre (BAM) website. A 2024 survey identified the need for more accessible and interactive tools. This has been interpreted into the initiative to create a dashboard that showcases avian model insights for improved conservation under the guidance of BAM. The current audience is made up of advanced users - both in their subject knowledge and model and use of advanced statistical software. A main objective is to increase this audience to a wider base.

As the current website does not allow for user input or interaction, this limits the ability of users to dive into the model outputs and effectively digest the current available information.

### Background & Importance

A mandate of BAM's is to support migratory bird management and conservation across the Boreal region of North America for 144 species of birds. They do this through several data-driven tools that can be used to address different conservation challenges. These applications include, but are not limited to:
...
The initial EDA conducted in *Jupyter Notebooks* and locally-run *Shiny App* can be reproduced by installing the conda [environment](https://github.com/UBC-MDS/Boreal-Birds/blob/main/environment.yml "environment.yml file") and following instructions in the [README](https://github.com/UBC-MDS/Boreal-Birds/blob/main/README.md "Boreal-Birds README")
...
Currently, these files are stored on a Google Drive. To improve loading speeds, we suggest pre-loading these model outputs to a SQL (or equivalent) database. This will enable fast retrieval and structured data validation. However, we will be discussing this with the BAM team to ensure long-term maintainability.
...
Success will also include the capstone partner satisfaction and enthusiasm with the final results. The overall goal is to incorporate the provided criteria, ideas, and vision into the new dashboard, as well as elaborating and expanding on its new capabilities.

![Evaluation & Success Criteria](images/clipboard-3029239191.png){#fig-eval-success width="665"}
